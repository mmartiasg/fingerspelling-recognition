...
class FingerSpellingV1(tf.keras.Model):
    def __init__(self, attention_heads,
                    embedding_dims,
                    encodder_kernel_size,
                    decoded_dropout_rate,
                    max_source_length,
                    max_target_lenght, 
                    vocab_size,
                    feature_columns,
        ...
        latent_dims = math.ceil(embedding_dims/attention_heads)
        ...
        self.encoder = TransformerEncoder(num_heads=attention_heads, dim_emb=embedding_dims, dim_dense=latent_dims)
        self.decoder = TransformerDecoder(num_heads=attention_heads, dim_dense=latent_dims, dim_emb=embedding_dims)
    ...
    def generate(self, source):
        ...
        for i in range(self.max_target_lenght - 1):
            ...
            logits = self.next_token_proba(dec_out, training=False)
